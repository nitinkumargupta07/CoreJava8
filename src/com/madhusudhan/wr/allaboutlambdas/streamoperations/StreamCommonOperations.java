...
public class StreamCommonOperations {
	...
	private void findExecutives(List<Employee> employees) {
		Stream<Employee> empStream = employees.stream().filter(Employee::isExecutive);
	...
	private void chainingFilters(List<Employee> employees) {
		Stream<Employee> empStream = employees.stream().filter(e -> e.getDepartment().equals("Marketing"))
	...
	private void upperCaseEmployees(List<Employee> employees) {
		Stream<String> employeeStream = employees.stream().map(Employee::getName).map(String::toUpperCase);
	...
	/**
	 * Flattening a stream of list of actors to a stream of actors using flatMap
	 * function
	 * 
	 * @param movies
	 */
	private void flatMapMovieActors(List<Movie> movies) {
		...
		Stream<List<Actor>> actorsListStream = movies.stream().map(Movie::getActors);
		actorsListStream.forEach(System.out::println);

		// Type mismatch: cannot convert from Stream<Actor> to Actor
		/*
		 * Stream<Actor> actorsListStream = movies .stream() .map(m ->
		 * m.getActors().stream()); actorsListStream.forEach(System.out::println);
		 */
	...
	/**
	 * Using flatMap function to flatten a stream of list of movies to stream of
	 * movies
	 * 
	 * @param movies
	 */
	private void flatMapMovies(List<Movie> movies) {
		...
		String[] thriller = new String[] { "Bourne Identity", "The Ghost Writer", "Clear and Present Danger" };
		...
		Stream<List<String>> actionMoviesStream = Stream.of(actionMoviesList, comedyMoviesList, thrillerMoviesList);
		...
		// With Flattening
		actionMoviesStream.flatMap(s -> s.stream()).forEach(System.out::println);
	...
	private void collectingToAList(List<Trade> trades) {
		List<Trade> bigTrades = trades.stream().filter(Trade::isBigTrade).collect(Collectors.toList());
	...
	private void collectingToASet(List<Trade> trades) {
		Set<Trade> bigTrades = trades.stream().filter(Trade::isBigTrade).collect(toSet());
	...
	private void collectingToAMap(List<Movie> movies) {
		...
		Map<String, List<Actor>> movieActorsMap = movies.stream()
	...
	private void distinctEmployees(List<Employee> employees) {
		Stream<String> employeesNamesStream = employees.stream().map(Employee::getName).distinct();
	...
	private void limitOutput(List<Employee> employees) {
		Stream<String> employeesNamesStream = employees.stream().map(Employee::getName).limit(3);
	...
	private void skipEmployees(List<Employee> employees) {
		Stream<String> employeesNamesStream = employees.stream().skip(2).map(Employee::getName);
	...
	private void findFirst(List<Trade> trades) {
		...
		Optional<Trade> firstBigTrade = trades.stream().filter(Trade::isBigTrade).findFirst();
	...
	private void anyMatch(List<Trade> trades) {
		boolean cancelledTrade = trades.stream().anyMatch(t -> t.getStatus().equals("CANCELLED"));
	...
	private void reducingTradeQuantity(List<Trade> trades) {
		Optional<Integer> totalQuantity = trades.stream().map(Trade::getQuantity).reduce((a, b) -> a + b);
	...
	private void reducingPrimitiveVersion(List<Trade> trades) {
		OptionalInt totalQuantity = trades.stream().mapToInt(Trade::getQuantity).reduce(Integer::sum);
	...
	private void reduceUsingSeed() {
		...
		int sum = ints.stream().reduce(10, (i, j) -> i + j);
